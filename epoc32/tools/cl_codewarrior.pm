...
sub PMCheckPlatformL {
	...
	if (defined($ENV{MWCSym2Includes})) {
		$MWCC = "mwccsym2.exe";
		$MWLD = "mwldsym2.exe";
		# print "Detected CodeWarrior for Symbian OS, OEM version 2\n";
	}
	if (!defined($MWCC) && defined($ENV{MWSym2Libraries})) {
		$MWCC = "mwccsym2.exe";
		$MWLD = "mwldsym2.exe";
		# print "Detected CodeWarrior for Symbian OS, Pro version 2.0\n";
	}
	if (!defined($MWCC) && defined($ENV{CWFolder})) {
		$MWCC = "mwcc.exe";
		$MWLD = "mwld.exe";
		# print "Assuming CodeWarrior for Symbian OS, Pro version 1.0\n";
	}
	if (!defined($MWCC)) {
...
# hack ends
	...
	if($VariantFile){
	    ...
	    # workaround for codewarrior bug:
...
#			stdexe/stddll hack so that the inline functions dont 
...
#tools hack end
  ...
  &Generic_MakeWorkDir("MAKEWORK$Bld", "$RelPath$adjustedTargetPath");
	...
	&main::OutFormat(
		...
		' -msgstyle gcc',
		' -stdlib'		# FIXME: use default runtime library for compiler help functions
	);
	if ($MWLD eq "mwcc.exe") {
		&main::OutFormat(
...
# tools hack end
	...
	if($TrgType =~/^(STDEXE|STDDLL|STDLIB)$/o) {
		if(! (( $CWVersion == 0x3205 && $CWBuildNum >= 465 ) || $CWVersion > 0x3205 ) ) {
			&main::FatalError("TargetType $TrgType requires CodeWarrior version `3.2.5 build 465' or later");
		}
	}
