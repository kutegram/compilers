...
my $MinGWPreInclude='$(EPOCINC)\\gcc_mingw\\gcc_mingw_3_4_2.h';
...
my %PlatOpt=(
	...
	'Optimize'=>'-fstack-check -fdefer-pop -fmerge-constants '.
...
#	foreach (@SysIncPaths, @UserIncPaths) {
#		&main::Output(
#			" -I \"$_\""
#		);
#	}
...
#	.map files not currently supported
#	if ($BasicTrgType=~/^(DLL|EXE)$/o) {
#		push @releaseables, "$LibPath$Trg.map";
#	}
...
#		call g++ to link the target
	...
	#	if (scalar @LibList or scalar @StatLibList)
	#	{
			&main::Output(
				"\t\t-L\$(EPOCTRG$Bld) \\\n"
			);
