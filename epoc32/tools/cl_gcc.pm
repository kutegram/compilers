...
#		code to ensure that the static libraries for all builds are built at the library stage
		...
		&main::Output(
			...
			&Generic_Quote("\$(EPOCBLD)\\$ExportLibrary.prep.def"), "\n",
			"\t$Dlltool $PlatOpt{Dlltool} --output-lib \"\$@\" --def \"\$<\" --dllname \"$LinkAs\"\n"
		);
		...
		foreach (@CompatibleABIs) {
			&main::Output(
				...
				&Generic_Quote($DefFile), "\n",
				"\t$Dlltool $ABIDlltool{$_} --output-lib \"$ABIRTWLibPath{$_}UREL\\$ExportLibrary.lib\" \\\n",
				"\t\t--def \"\$(EPOCBLD)\\$ExportLibrary.prep.def\" \\\n",
				"\t\t--dllname \"$LinkAs\" \n"
			);
			if ($ExtraExportLibrary) {
				&main::Output(
...
#			call perl on the script here so nmake will die if there are errors - this doesn't happen if calling perl in a batch file
	...
	if ($DefFile and !$NoExportLibrary) {
		&main::Output(
			...
			"\t-\$(ERASE) \"\$(EPOCBLD)\\$ExportLibrary.prep.def\"\n"
		);
		if ($ExtraExportLibrary) {
			&main::Output(
...
#		generate a .DEF file from the objects and static libraries
		&main::Output(
			"\t$Dlltool $PlatOpt{Dlltool} --output-def \"\$(EPOCBLD$Bld)\\$ExportLibrary.inf\" \"\$(EPOCBLD$Bld)\\$BaseTrg.in\""
		);
		foreach (@StatLibList) {
			&main::Output(
				" \"\$(EPOCSTATLINK$Bld)\\$_\""
...
#		reorder the .DEF file taking frozen exports into account if there are any
		&main::Output(
#			call perl on the script here so nmake will die if there are errors - this doesn't happen if calling perl in a batch file
			"\tperl -S makedef.pl -Deffile \"\$(EPOCBLD$Bld)\\$ExportLibrary.inf\"$AbsentSubst"
		);
		if (SystemTarget()) {
...
#		generate an export object from the ordered .DEF file
		&main::Output(
			"\t$Dlltool $PlatOpt{Dlltool} --def \"\$(EPOCBLD)\\$ExportLibrary.def\" \\\n",
			"\t\t--output-exp \"\$(EPOCBLD$Bld)\\$ExportLibrary.exp\" \\\n",
			"\t\t--dllname \"$LinkAs\""
		);
		if (&main::ExportUnfrozen) {
			&main::Output(
				"\\\n",
				"\t\t--output-lib \"\$(EPOCLIB)\\UREL\\$ExportLibrary.lib\"",
				"\n"
			);
			if ($ExtraExportLibrary) {
				&main::Output(
...
#	call ld to do base relocations (and dll exports)
	if ($BasicTrgType=~/^(DLL|EXE)/o) {
		...
		if ($BasicTrgType=~/^DLL$/o) {
			&main::Output(
				" $PlatOpt{Entry} $EntrySymbol -u $EntrySymbol \"\$(EPOCBLD$Bld)\\$ExportLibrary.exp\" --dll \\\n"
			);
		}
		elsif ($BasicTrgType=~/^EXE$/o) {
			&main::Output(
...
#		delete temporary files
		if ($BasicTrgType=~/^DLL$/o || $TrgType=~/^EXEXP$/o || $TrgType=~/^EXEDLL$/o) {
			&main::Output(
				"\t-\$(ERASE) \"\$(EPOCBLD$Bld)\\$ExportLibrary.exp\"\n"
			);
		}
		&main::Output(
...
#		delete temporary files
		...
		if (&main::PagingTargetMode == UNPAGED) {
			&main::Output(
				' -unpaged'
			);
		}
		elsif (&main::PagingTargetMode == PAGED) {
			&main::Output(
				' -paged'
			);
		}
