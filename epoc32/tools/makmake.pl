...

{
	...
	unless (GetOptions(\%Options, 'd', 'mmp', 'plat=s', 'v', 'arm', 'nd' , 'ithumb' , 'iarm' , 'debug', 'no_debug', 'logfc','wrap')) {
		exit 1;
	}
	...
	if($Options{wrap})
	{
		...
		$IsCompilerWrapperOption = $Options{wrap};
	}
...
my $PagingTargetMode;	# 0-N/A, 1-UNPAGED, 2-PAGED
...
if ($FeatureVariantArg)
	...
	if (${%{&Mmp_TrgType}}{Basic} =~ /^(EXEDLL|EXE|DLL|LIB)$/ && &Mmp_IsFeatureVariant)
		{
...
if (${%{&Mmp_TrgType}}{Basic} =~ /^(EXEDLL|EXE|DLL|LIB)$/ && %FeatureVariantInfo && $FeatureVariantInfo{INVARIANT} && $FeatureVariantInfo{NAME} !~ /^default$/i)
	{
	...
	# Open the variant makefile
	open MAKEFILE,">$MAKEFILE" or die "ERROR: Can't open or create file \"$MAKEFILE\"\n";

	# Generate the default makefile name
	$MAKEFILE =~ s/\.$FeatureVariantInfo{NAME}$/\.default/;

	# If the default makefile doesn't exist it will need to be created in which case we need the platform makefile name
	my $plat_makefile = &Path_Chop($E32env::Data{BldPath}) . $Path{BldInfPath} . $Plat{Real} . '.default.make';
	my $plattest_makefile = &Path_Chop($E32env::Data{BldPath}) . $Path{BldInfPath} . $Plat{Real} . '.defaulttest.make';  
	my $plat_target = 'MAKEFILE' . &Path_Split('Base',$MMPFILE);

	# Pattern match any target whatsoever with a prerequisite on the default makefile
	# Then just pass the target into the default makefile to process
	print MAKEFILE "\%: $MAKEFILE\n";
	print MAKEFILE "\t\@\$(MAKE) -f \"$MAKEFILE\" \$\@\n";

	# Generate the rule to make the default makefile if it doesn't exist
	print MAKEFILE "\n$MAKEFILE:\n";
	print MAKEFILE "\t\@\$(MAKE) -i -s -f \"$plat_makefile\" -f \"$plattest_makefile\" $plat_target > nul 2>&1\n";
...

{
	...
	if ($TrgType{Name} eq 'STDEXE' or $TrgType{Name} eq 'STDDLL' or $TrgType{Name} eq 'STDLIB') {
		my $inc=$E32env::Data{EPOCPath}."include\\stdapis";
		push @SysIncPaths, $inc;
	}
...
#	rombuild
	...
	unless ($TrgType{Basic} =~ /^IMPLIB$/io or $TruePlat{Ext} =~ /\.DSP|\.xml/i) { # hack to avoid rombuild target for IDE makefiles
		&Output("ROMFILE:\n");
...
#				}
				...
				if ( $PagingTargetMode==UNPAGED) {
					$RomDecorations.=' unpaged';
				}
				elsif ($PagingTargetMode==PAGED) {
					$RomDecorations.=' paged';
				}
...
sub SetVarsFromMmp ($) {
	...
	$PagingTargetMode=&Mmp_PagingTargetMode;
...
#	this is the path where the target puts it's import library
	...
	if ($CurBld) {
#regression hack
		if (($Plat{DefFile} eq 'EABI') || ($Plat{OS} eq 'TOOLS2')) {
			$Path.="lib\\";
		}
...
sub Trg (;$) {
	...
	my $localTrg = $Trg;	
	$localTrg =~ s/(\.\S+)$/\.$FeatureVariantInfo{$bld."_LABEL"}$1/ if ($bld && %FeatureVariantInfo && !$FeatureVariantInfo{INVARIANT} && $FeatureVariantInfo{$bld."_LABEL"});
	return $localTrg;
}
sub TrgPath () {
	$TrgType{Path};
