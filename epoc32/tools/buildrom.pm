...
sub print_usage
{
	...
	if ($enforceFeatureManager) 
	{
		$featuresOptionUsage = "-fm=featuredatabasefile          -- feature manager/feature registry database XML file name.\n".
							   ...
							   "   -nofm|-nofm=featuresdatafile     -- don't generate features data file.".
							   " Instead use pre-built features data file if provided.";
	}
...
The available options are
   ...
   -loglevel<level>                 -- Level of information logging where loglevel is 0,1
...
#		$image_content = $1;
#		$xmlrequired = 1;
#		print "image content file name $image_content \n" if ($opt_v);
		...
		if ($arg =~ /^-nofm(=(.*))?$/)
		{
			...
			$noFeatureManager = 1;
			$preBuiltFeaturesDataFile = $2 if($2);						
			next;	
		}
	...
	{
		print "Error: Option \"-fm\" is mandatory unless option \"-nofm\" is provided\n";
		$errors++;
	}
...
sub preprocessing_phase
{
	...
	#Macro "ROM_FEATURE_MANAGEMENT" is defined when "-f" or "-i" is used
	#if ( (defined ($image_content)) && (defined ($featureXml)) ) 
	...
	while ($line=<TMP1>)
	{
		...
		if ($line=~/^\s*DATA_IMAGE\s+/i)
		{
			if ($line =~ /\s+(\d+)\s+(\S+)\s+/i)
			{
				...
				my $compress=0;
				my $uncompress=0;
				if ($line =~ /\s+compress\s*/)
				{ 
					$compress=1;
				}
				elsif($line =~ /\s+uncompress\s*/)
				{ 
					$uncompress=1;
				}
				if ($line =~ /\s+fat16\s*/)
		...
		if ($line=~/^\s*ROM_IMAGE\s+/i)
		{
			if ($line =~ /\s+(\d+)\s+(\S+)\s+/i)
			{
				...
				if ($line =~ /\s+extension\s*/)
					{ $extend=1; }
				if ($line =~ /\s+composite_primary\s*/) # added to support new composite_primary keyword in obey files
				{	if (!($extend))
						{ $composite="composite_primary"; }
					else
						{ print "Error: composite_primary keyword must be used with a core image\n"; }
				}
				if ($line =~ /\s+composite_secondary\s*/) # added to support new composite_secondary keyword in obey files
				{ if (!($extend))
...
sub reorganize_phase
{
	...
	foreach $line (@obydata)
	{
		...
		elsif( $line =~/^\s*dataimagename\s*\=\s*(\S+)/ )
		{
		...
		elsif( $line =~/^\s*dataimagesize\s*\=\s*(\S+)/ )
		{
		...
		elsif( $line =~/^\s*dataimagefilesystem\s*\=\s*(\S+)/ )
		{
...
sub multlinguify_phase
{
	...
	foreach $line (@obydata)
	{
		...
		elsif ($line=~/^(.*?)\bMULTI_LINGUIFY\s*\(\s*(\S+)\s+(\S+)\s+(\S+)\s*\)(.*)$/)
		{
			my $initialStuff=$1;
...
sub fallback
{
	...
	my @BPABIPlats = &BPABIutl_Plat_List; # Add the BPABI Platforms to be added
...
sub bitmap_aif_converison_phase
{
	...
	foreach $line (@obydata)
	{
		...
		elsif ($line =~ /^\s*compressed-bitmap=\s*"?(\S+)"?\s+"?(\S+)"?/)
		{
			my $mbm = $1;
			my $dest = $2;
...
sub reformat_line($)
{
	...
	elsif($line =~ /^\s*(patchdata)\s*(\S+)\s*\@\s*(\S+)\s+(\S+)/)
	{
...
{	
	...
	foreach $line (@obydata)
	{
		...
		push @newobydata, reformat_line($line);
	}
				...
				for(;$i<scalar @hidearray;$i++) {
					if($hidearray[$i]{hide}==1) {
						...
						while($obeyrom<$rom && $obyrecordline<scalar @obydata) { #pushes lines to @newobydata until specified rom is reached
							...
							if($obydata[$obyrecordline] =~/^\s*REM\s+ROM_IMAGE\[(\d)\]\s*$/){
								$obeyrom=$1;
							}
							$obyrecordline++;
						}
...
sub invoke_rombuild
{
	my $rombuild;
	if(!$geninc){
		$rombuild = "rombuild -slog $logLevel";
	}
	else
	{
		$rombuild = "rombuild -slog -geninc $logLevel";
	}
	my $rofsbuild = "rofsbuild -slog $logLevel";
	...
	foreach my $dll (keys %DllDataMap){
		...
		if($rootPlatName =~ /^ARMV5|$plat$/i){
			...
			my $line = $_;
				...
				$symbolTblEntry = $line;
				if($symbolTblEntry =~ /\s*(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(.*)/){
					my $symbol		= $1;
					my $symbolValue = $2;
					my $data		= $3;
					my $symbolSz	= $4;
