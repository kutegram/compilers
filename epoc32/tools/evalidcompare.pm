...
my %typeHandler = (
      ...
      elf => {reader => 'elfdump -i', filter => \&ElfDumpFilter},
      intel => {reader => 'nm --no-sort', filter => \&NmFilter, dos_paths => 1},
      intel_pe => {reader => 'pe_dump', filter => \&FilterNone},
      ...
      permanent_file_store => {reader => 'pfsdump -c -v', filter => \&PermanentFileStoreFilter, relative_paths => 1},
...
sub WriteFilteredData {
  ...
  if (exists $typeHandler{$type}->{expandor})
  {
    ...
    my $raw;
    if ($reader) {
      $raw = IO::File->new("$reader \"$file\" 2>&1 |") or die "Error: Couldn't run \"$reader $file\": $!\n";
    }
    else {
...
sub DoCompareFiles {
  ...
  if ($type ne 'identical')
  {
    ...
    my $fileHandle2;
    if ($reader) {
      $fileHandle1 = IO::File->new("$reader \"$file1\" 2>&1 |") or die "Error: Couldn't run \"$reader $file1\": $!\n";
      $fileHandle2 = IO::File->new("$reader \"$file2\" 2>&1 |") or die "Error: Couldn't run \"$reader $file2\": $!\n";
    }
    else {
...
sub Elf2E32Filter {
  ...
  if ($$line =~ /imports from /) {
  	$$line = lc $$line;	# DLL names are not case-sensitive in the Symbian OS loader
  }
}
...
sub ElfDumpFilter {
  ...
  $$line  =~ s/#<DLL>(\S+\.\S+)#<\\DLL>/#<DLL>\L$1\E#<\\DLL>/; # DLL names are not case-sensitive in the Symbian OS loader
  if ($$line =~ /^\.(rel\.)?debug_/) {
