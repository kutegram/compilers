			...
			if ($currdecl[0] ne '(') {
				...
				if ($ind1>0) {
					...
					if ($type2->{basic}) {
						...
						$size = $type2->{size};
						$category = 'basic';
#						print "BASIC: alias=$alias size=$size cat=$category\n";
					} elsif ($type2->{enum}) {
						$alias = $type2->{name};
						$category = 'enum';
						$size = 4;
		...
		if ($size>0) {
			...
			if ($array>0) {
				$size = ($size + $am) &~ $am;
				if ($typedef) {
#					print "TYPEDEF: name=$ident, category=$category, alias=$alias, size=$size*$array, spacing=$size, array=$array\n";
					push @$typedefs, {name=>$ident, category=>$category, alias=>$alias, size=>$size*$array, spacing=>$size, array=>$array};
				} else {
#					print "MEMBER: name=$ident, size=$size*$array, offset=$curr_offset, spacing=$size\n";
					push @$members, {name=>$ident, size=>$size*$array, offset=>$curr_offset, spacing=>$size};
				}
				$size *= $array;
			} else {
				if ($typedef) {
#					print "TYPEDEF: name=$ident, category=$category, alias=$alias, size=$size\n";
					push @$typedefs, {name=>$ident, category=>$category, alias=>$alias, size=>$size};
				} else {
#					print "MEMBER: name=$ident, size=$size, offset=$curr_offset\n";
