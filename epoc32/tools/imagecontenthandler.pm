...
#	&PrintMsg ($indent. &genericparser::getElementName($ImageNode) ."{$childCnt} {$ImageDetailRef}\n");
...
#			&PrintMsg ("*".$indent . $branch. " ". $attrib->getName ."=" . $attrib->getValue ."\n");
		}
...
#					&PrintMsg ("Available index is $availablePos \n");
					last;
				}
				$availablePos++;
			}
...
#	printMsg("Binary Selection..@{$ImageContent{options}{binaryselectionorder}{vals}}\n");
...
#	&PrintMsg ("*********Processing target ". &genericparser::getAttrValue($trgNodesRef->[0], "imageid"). "=>" . $romGeometry[$ImgPos]{name} ."****\n") ;
	my @cdfFileList;
...
#		&PrintMsg ("Feature $feature $IncExcStr" . "d \n");
...
{
	my ($binaryName) = @_;
#	if( !(grep $binaryName, @{$ImageBinaryList[$curRomImageIndex]} ))
	{
#		&PrintMsg ("=binary $binaryName added at ROMIMAGE[$curRomImageIndex]\n");  #=>". ${&GetBinaryInfo($binaryName)}{source}."\n");
...
#		foreach $key (keys %DefaultDirs)
#		{
#			&PrintMsg ("$key=$DefaultDirs{$key}\n");
#		}
	}
	else
	{
...
#			&PrintMsg ("new obey  file entry ". $binary );
...
#					stack,heap,fixed,priority,uid,dll,dlldatatop
				...
				else
				{
					...
					elsif($type =~ /compressedbitmap/)
					{
						$line = "compressed-bitmap=";
					}
					elsif($type =~ /autobitmap/)
					{
						$line = "auto-bitmap=";
					}
					elsif($type =~ /bitmap/)
...
#	if( defined @{$ImageContent{options}{kernel}{single}{vals}})
#	{
#		$line = "singlekernel\n";
#		print $fh $line;
#	}
...
sub LoadFromCDF
{
#	&ImageContentHandler::PrintMsg( "within LoadFromCDF()\n");
	my $cdf;
	my $binFile;
...
#	&ImageContentHandler::PrintMsg( "Load from deplister ".  $cdf. "\n");
	cdfparser::LoadCDF($cdf);
#	&ImageContentHandler::PrintMsg( "Loaded " . $cdf. "\n");

#Get all binaries from the mdf
	(@BinList) = &cdfparser::GetBinaries($cdf);

#	&ImageContentHandler::PrintMsg ("...". scalar @BinList . "\n");
	...
	foreach $aBinary (@BinList)
	{
		...
		{
#			&ImageContentHandler::PrintMsg( "$indent Binary1 $aBinary already found\n");
			next;
		}
		else
		{
			$VisitedBinaryInfoHash = \%{$VisitedBinaries{$aBinary}};
#			&ImageContentHandler::PrintMsg( "$indent Binary1 $aBinary (adding)\n");
		}
...
#			&ImageContentHandler::PrintMsg( "Dyn Dep on $aFile\n");
...
#		&ImageContentHandler::printMsg( ">>>>>>>>>>>>>cdf binary file name $binInfo->{source}\n");
...
#default abi directory. Otherwise, fallback to the default ARMV5 directory.
			...
			if($foundFile == 0)
			{
#				ImageContentHandler::PrintMsg ("Error: $aBinary: $aAbsFile No such file found\n");
				return;
			}
...
#		Mark the binary it it is a valid E32 executable.
	...
	foreach my $aFile (@StatDepsList)
	{
		...
		if(defined($new_cdf))
		{
#			&ImageContentHandler::PrintMsg( "cdf file $new_cdf for binary file $aFile\n");
			LoadFromCDF($new_cdf, $aFile);
		}
		else
		{
#			Include the static dependencies even if there is no mdf describing this binary
			...
			{
#				&ImageContentHandler::PrintMsg( "Static deps for $aFile is already checked....\n");
			}
		}
	}
}
