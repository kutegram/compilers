# IDE_CW.PM
#
# Copyright (c) 2001-2008 Symbian Software Ltd.  All rights reserved.
#
...
#$GCCE_CompilerOption .= " -fvisibility=default";
...
#	print ("Detected : CW For Symbian OS $CW_major_version.$CW_minor_version\n");
...
#			$PlatOpt{Entry}='--thumb-entry';	ALL ENTRYPOINTS ARE ARM ON EKA2
...
# hack - only use colour resource files
...
#  		addFile($rssfile, "Text", "", "", "Resources");
   	}
	...
	foreach $file (@StatLibList)
		{
		next if ( $file eq $FirstLib );		#skip if file equals FirstLib.
		if ($Plat eq "GCCE" or $Plat eq "ARMV5_ABIV2") {
			$file =~ s/\.lib$/.dso/;
			$file =~ s/\.LIB$/.DSO/;
			}
		addFile($file, "Library", $debug, "$Bld\\"); # static library, build specific
	...
	if ($Plat eq "GCCE" || $CustGCCE)
	{
		foreach $file (@RuntimeLibs)
		{
			...
			#hack to prevent multiple libs being listed when they are shared between targets.
...
#############################
		...
		if ($Bld =~ /REL$/)
			{
				$compilerargs .= $configdata{"REL_OPTIMISATION"}.$CCUREL;		
			}
		else
			{
			unless (&main::SrcDbg)
				{
					$compilerargs .= $configdata{"DEBUG_OPTIMISATION"}.$CCUDEB;		
				}
			}
...
############################
		...
		{
		$compiler = "ARM GCCE";
		#HACK.... setting CompilerXMLDescriptor is only good for CW 3.0 and greater.
...
####################
	...
	$changedsettings{"Compiler"} = $compiler; # HACK: in CW 3.0, "Compiler" no longer exists. This line has no effect on those versions
...
#		    If platform is a customization, take libs from parent directory.		   
			...
			# hack - exexps are allowed data, but they look like dlls to elftran....
...
#		addLinkDescriptorSymbol ('${var:IMPLIBS_NO_PATH_VTBLEXPORT}', $implibs_no_path_vtblexport);
		...
		$Link = 'armlink '.$oP.'diag_suppress 6331'.$linkeropts.' ';
...
# drop the --noscanlib until ARM fix the defect :-(		
#	        $linkCommand = "$Link ".$oP.'bpabi '.$oP.'reloc '.$oP.'split '.$oP."rw-base $datalinkbase ".$oP.'noscanlib '."$PlatOpt{Ld}";
	        $linkCommand = "$Link ".$oP.'bpabi '.$oP.'reloc '.$oP.'split '.$oP.'no_scanlib '.$oP."rw-base $datalinkbase "."$PlatOpt{Ld}";
			$linkCommand .= ' --dll --symver_soname --soname ${output.file.root}{000a0000}['.$UidList[2].']${output.file.extension} ';
...
#		    If platform is a customization, take libs from parent directory.		   
		    ...
		    {
		    # TODO: ARMV5 hardcoded here...
		    $linkCommand .= $oP.'entry _E32Dll "'.$FirstLib.'('.$ObjFile.')"';
			#$linkCommand .= $oP.'entry _E32Dll "${output}\\'.$FirstLib.'('.$ObjFile.')"'.
			#		' "${target.data}\\'.$tempFilenameRoot.'.exp"';
		    }
		    ...
		    # TODO Should this use $ParentPlat rather than hardcoded ARMV5 dir?
		   $linkCommand .= $oP.'entry _E32Startup "'.$FirstLib.'('.$ObjFile.')"';
...
# TOOD: prolly add in .map file output for urel
#			$linkCommand .= ' '.$oP.'symbols '.$oP.'list "${output}\\${output.file.name}.map"';
#			$linkCommand .= ' "${target.data}\\${output.file.root}.in"';
		...
		#	if($IsPlatCustomization) 
		#	{
		#	     $linkCommand .= ' --userlibpath "${var:KIT_EPOCROOT}\\EPOC32\RELEASE\\'.$ParentPlat.'\\'.$Bld.'","${var:KIT_EPOCROOT}\\EPOC32\RELEASE\\'.$ParentPlat.'\LIB" '.$oP.'via "${target.data}\\${output.file.root}_'.$Bld.'_objects.via" ${var:RUNTIME_LIBS}';
		#	}
		#	else
		#	{
# why adding runtime libs twice here?
#				addLinkDescriptorSymbol ('${var:RUNTIME_LIBS}', 'usrt'.$RVCTVersion.'.lib dfpaeabi.dso  dfprvct'.$RVCTVersion.'.dso drtaeabi.dso scppnwdl.dso drtrvct'.$RVCTVersion.'.dso');
		#	}
				
			$linkCommand .= ' --userlibpath "${var:KIT_EPOCROOT}\\EPOC32\RELEASE\ARMV5\\'.$Bld.'","${var:KIT_EPOCROOT}\\EPOC32\RELEASE\ARMV5\LIB" '.$oP.'via "${target.data}\\${output.file.root}_'.$Bld.'_objects.via" ${var:RUNTIME_LIBS} ';
		...
		# TODO: Section needs to be generic for BPABI (e.g. GCCE & ARMV5_ABIV2)
		...
		$linkCommand .= ' --linkas=${output.file.root}{000a0000}['.$UidList[2].']${output.file.extension}';
		#HACK - LIB path is hardcoded here...
...
###############################################
	...
	{	
		...
		$linkCommand .= ' -shared --default-symver -soname ${output.file.root}{000a0000}['.$UidList[2].']${output.file.extension}';
