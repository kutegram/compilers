...
#	Process options and check that all are recognised
	unless (GetOptions(\%Options, 'check|c', 'keepgoing|k', 'savespace|s', 'verbose|v',
						'what|w', 'remove|r', 'instructionset|i=s',
						'checksource|cs', 'debug','no_debug', 'logfc|fc','wrap')) { 
		exit 1;
	}
...
#				report the error
	...
	if($Options{wrap})
	{
		...
		{
			$CompilerWrapperFlagMacro =" ABLD_COMPWRAP_FLAG=-wrap";
		}
...
#						hack for TOOLS, TOOLS2 and VC6TOOLS platforms
						if ($Plat ne 'ALL') {
...
#			set up a list of builds to carry out commands for if appropriate
			...
			if (${$Commands{$Command}}{build}) {
				if ($Bld eq 'ALL') {
					unless ($Plat=~/TOOLS2?$/o) { # hack for platforms TOOLS, TOOLS2 and VC6TOOLS
						@Blds=('UDEB', 'UREL');
					}
					else {
						@Blds=('DEB', 'REL');
...
Options (case-insensitive) :
  ...
  -wrap                 enable invocation of compiler wrapper tool

 possible combinations :
	(([-check]|[-what])|([-k][-s][-v][-i [thumb|arm]][-cs][-debug|-no_debug][-fc][-wrap]))
ENDHERESTRING
...
sub GetMakefileVariations ($$)
	...
	if (!$featureVariantArg)
		...
		push @variations, ".".$_ foreach (@resolvedVariants);
		}
