...
sub _CheckContextTag {
	my $self = shift;
	# Check that context doesn't contain any UTF encoded characters.
	my $nodes = $self->_iAsptmlDom->getElementsByTagName("context");
	my $n =	$nodes->getLength();
	for (my $i = 0; $i < $n; $i++)
		{
		my $contextUID = $nodes->item($i)->getAttributeNode("contextUID")->getValue;
			
		if ($contextUID =~ /\&\#\d{3,}\;/)
			{ #UTF tags not allowed in contextUID
			$self->_iLog->LogDie("context contains $contextUID.\nSorry: context	must only contain ASCII characters a-z, A-Z, _ and 0-9.\n");
			}
		}
}
...
################################################################################
#
# Function CheckContentOfTags
#
# Function Validates the contents of certain tags. Right now just context.
#
...
# Notes:
################################################################################
sub CheckContentOfTags() {
	my $self = shift;
	$self->_CheckContextTag;
}
