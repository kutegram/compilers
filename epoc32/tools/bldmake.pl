...
my %Options;
my %KeepGoing;
my @DefaultPlats=('WINSCW', 'GCCXML', 'EDG');
my @BaseUserDefaultPlats=('ARM4', 'ARM4T', 'WINSCW', 'GCCXML', 'EDG');
...
# Need to add ARMV4 and ARMV5 if RVCT compiler is present
# Use $ArvctVersion to determine the presence of the compiler

my $ArvctVersion=&Armutl_ArmVersion;
push @BaseUserDefaultPlats, 'ARMV4', 'ARMV5' if ($ArvctVersion);
...
if ($variantABIV2Keyword && $ArvctVersion) {
...
foreach my $BPABIPlat (@BPABIPlats) 
{
	...
	if(($BPABIPlat =~/^ARMV5/i))
		{
			if(!($BPABIPlat =~/^ARMV5$/i) && $ArvctVersion)
			{
...
#	process the commmand-line
	unless (GetOptions(\%Options, 'v', "k|keepgoing", "notest")) {
		exit 1;
	}
	unless (@ARGV>=1) {
		&Usage();
...
#       Add Customizations
 ...
 #	Force GCCXML support for anything that compiles as ARMV5
	if ( (grep /^ARM4$/, @RealPlats) and not (grep /^GCCXML$/,@RealPlats) ) 
			{
...
#	storing the information in a big data structure
	...
	if ($Command eq 'BLDFILES') {
		...
		# if compiling for ARMV5 then make sure version of RVCT
		# installed is correct.
		foreach my $plat (@DoPlats) {
			my $realplat = $Real{$plat};
			
			if ($realplat=~/^ARMV5/) {
				unless ($ArvctVersion >= 2.2) {
...
# Include the list of BPABI Platforms, so that they are added to the list of Real Platforms.
# However, build should happen for these platforms only if they are specified in the platform list.
...
# 	exclude GCCXML, EDG and CWTOOLS  from list of RealPlats
	...
	unless (($Plat =~ /^gccxml/i)  or  ($Plat =~ /^edg/i) or  ($Plat =~ /^cwtools/i)) {
...
# The BPABI top level makefiles should include the compiler configuration files
	...
	elsif ($CustomizesPlat && grep /^$CustomizesPlat$/i, @BPABIPlats) {
		my $config_file = BPABIutl_Config_Path($CustomizesPlat);
		&Output(
			"#This is tool chain config file\n",
			"include $config_file\n",
			"export PLAT=${Plat}\n\n"
...
#		standard commands
		unless ($$Ref{Makefile}) {
			...
			my $RealMakefile="-f \"$MakefilePath$$Ref{Base}.$RealPlat$FeatureVariant\"";
...
#		calls to custom makefiles
			...
			$MakefileCall.=join('', &Path_Chop($E32MakePath),"\\",$$Ref{Base}, "\\", $RealPlat);
