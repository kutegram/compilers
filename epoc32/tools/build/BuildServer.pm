...
my $gRealTimeError = 0;       # 0 = FALSE (No error) # Windows error code = TRUE
...
sub rcvd_msg_from_client {
    ...
    if ( $iCommand eq "Results")
    {
        ...
        if($gScanlogAvailable and Scanlog::CheckForRealTimeErrors($args))
        {$gRealTimeError = 240}; # Windows Error 240: The session was canceled.

        # If this is a critical step then flag a real time error
        if($gScanlogAvailable and
           $gCommands[$iID]{'ExitOnScanlogError'} =~ /y/i and
           Scanlog::CheckForErrors($args) )
        {$gRealTimeError = 240};
...
sub Start
{
  ...
  Msg->new_server($iHost, $iPort, \&login_proc, $iConnectionTimeout, $iSocketConnections);
...
sub AddReady
{
  ...
  if (scalar @iClientsWaiting == $iConn->TotalConnections)
  {
    ...
    if ($gRealTimeError)
    {
      ...
      print $gLogFileH "ERROR: Realtime error! BuildServer terminating\n";
      close ($gLogFileH);
      die "ERROR: Realtime error! BuildServer terminating\n";
    }
    ...
    if ($gExit)
    {
      ...
      {
        my $msg = freeze ("Exit");  
        $gClientHandles{$key}->send_now($msg);
      }
      exit 0;
    }
