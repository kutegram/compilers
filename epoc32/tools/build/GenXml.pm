...
# Set the appropriate version number
  ...
  {
    if ($docs[$i]->getDocumentElement->getAttribute('schema') =~ "2.0.0" && 
...
sub get_configuration_units2 ($$$$)	{
	...
	foreach my $element (@filterable_items)	{
		...
		if ($class eq "placeholder")	{
			# test to ensure this is actually empty and raise an error if not
			my @children = $element->getElementsByTagName("unit");
			if(scalar(@children))
				{
				print $GenXml::gLogFileH "ERROR: placeholder \"$name\" has units.\n";
				}
