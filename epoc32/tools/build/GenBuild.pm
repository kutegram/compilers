...
sub PBuildLevels
{
  ...
  if ($GenBuild::build_bootstrap)
  {
    ...
    if ($ENV{'Platform'} eq 'beech')
    {
      ...
      print $GenBuild::XMLFileH qq{\t\t<Execute ID="$GenBuild::iIDCount" Stage="$GenBuild::iStageCount" Component="BootStrap" Cwd="$GenBuild::iSourceDir}.qq{cedar\\generic\\tools\\e32toolp\\group" CommandLine="setupprj.bat"/>\n};
    } else {
    ...
    if ($ENV{'Platform'} eq 'beech')
    {
      ...
      print $GenBuild::XMLFileH qq{\t\t<Execute ID="$GenBuild::iIDCount" Stage="$GenBuild::iStageCount" Component="BootStrap" Cwd="$GenBuild::iSourceDir}.qq{cedar\\generic\\tools\\e32toolp\\group" CommandLine="bld.bat rel"/>\n};
    } else {
