...
sub Mmp_Reset() {
	...
	$PagingTargetMode=0;	# default N/A
...
# process the data structure
	...
	LINE: foreach $Line (@Mmp2D) {
		...
		if( /^PAGED$/o) {
			push @MmpDie, "$CurFile($LineNum) : paging mode (paged or unpaged) specified multiple times" if ($PagingTargetMode != NOTPAGED);
	...
	if ($PagingTargetMode == PAGED && $CompressTarget == COMPRESS) {
...
#		separately either
		...
		if ($Def{CheckSource_MMPEntry})
		{
			...
			# for GCCXML ignore check source errors for missing GCC .def file
			my $checkdef = (($IgnoreMissingDef) && ($Def{Path} =~ /[\\\/]bmarm[\\\/]/)) ? 0 : 1;
