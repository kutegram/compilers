  ...
  $(if $(wildcard $1),\
    $(eval __i_def2var := $(shell $(PERL) -Xe '\
      print(join(q(|), map(/^\s*\#define\s+(\S+)\s*(.*?)\s*$$/ ? qq($$1?=) . ($$2 eq q() ? 1 : $$2) : (),\
        sort({lc($$a) cmp lc($$b)} qx$(pchar)$(CPP) -nostdinc -undef -dM $(call dir2inc,$2) $1$(pchar)))))'))\
...
TARGETNAME := $(word 1,$(MAKECMDGOALS))
TARGETID   := $(if $(findstring _,$(TARGETNAME)),$(lastword $(subst _, ,$(TARGETNAME))),XX)
...
CPPFILE_LIST =
MKFILE_LIST  = $(call findfile,$(BUILD_INFOMK) $(BUILD_NAMEMK),$(FEATVAR_IDIR)) $(VARIANT_MK)
...
CMDFILE = $(WORKPREFIX)$(if $(notdir $(WORKPREFIX)),_)$(call substm,* : ?,@,$(TARGETNAME)).icmd
...
IMAKER_VARXML  = $(if $(IMAGE_TYPE),$($(IMAGE_TYPE)_PREFIX)_$(TARGETNAME)_config.xml)
...
IMAKER_EVAL = $(strip\
  $(foreach file,$(CPPFILE_LIST),$(call cppdef2var,$(file),$(FEATVAR_IDIR)))\
  $(foreach file,$(MKFILE_LIST),$(eval -include $(file)))\
  $(LANGUAGE_EVAL)\
...
include $(addprefix $(IMAKER_DIR)/imaker_,$(addsuffix .mk,help image public tools version))
