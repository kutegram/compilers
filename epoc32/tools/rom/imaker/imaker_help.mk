...
BUILD_HELP =\
  ...
  $(eval __i_help := $(shell $(PERL) -e 'eval {\
    (my $$re = q$(ichar)$(__i_var)$(ichar)) =~ s/(.)/{q(*)=>q(.*),q(?)=>q(.),q([)=>q([),q(])=>q(])}->{$$1} || qq(\Q$$1)/ge;\
    print(map(qq( $$_), sort({lc($$a) cmp lc($$b)} grep(/^$$re$$/, split(/\s+/, q$(ichar)$(__i_list)$(ichar))))))}'))\
  $(call peval,\
    my @var = ($(foreach var,$(__i_help),{\
      n=>$(call pquote,$(var))\
