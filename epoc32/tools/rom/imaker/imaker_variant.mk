...
VARIANT_OUTDIR   = $(call select,$(USE_VARIANTBLD),u,$(UDA_DATADIR),$($(call select,$(USE_VARIANTBLD),1,CORE,ROFS$(USE_VARIANTBLD))_DIR)/variant)
VARIANT_MKNAME   = variant.mk
VARIANT_MK       = $(if $(VARIANT_DIR),$(wildcard $(VARIANT_DIR)/$(VARIANT_MKNAME)))

VARIANT_PREFIX   = $(call select,$(USE_VARIANTBLD),1,$(CORE_PREFIX)_core,$(call select,$(USE_VARIANTBLD),u,$(UDA_PREFIX)_uda,$(ROFS$(USE_VARIANTBLD)_PREFIX)_rofs$(USE_VARIANTBLD)))
...
$(foreach rofs,2 3 4 5 6,\
  $(eval variantrofs$(rofs) variantrofs$(rofs)_%: IMAGE_TYPE = ROFS$(rofs))\
  $(eval variantrofs$(rofs) variantrofs$(rofs)_%: USE_ROFS = $(rofs))\
  $(eval variantrofs$(rofs) variantrofs$(rofs)_%: USE_VARIANTBLD = $(rofs))\
)
variantrofs%:\
  ;@$(call IMAKER,$$(ALL.ROFS$(USE_ROFS).STEPS))
