...
sub GlobFiles($)
{
    ...
    my @files = ();
    while ($file =~ /"+(.+?)"+|((\\\s|\S)+)/g) {
        local $_ = (defined($1) ? $1 : $2);
        push(@files, glob(/\s/ ? "\"$_\"" : $_));
    }
...
sub Search($$$$\@\$)
{
    my ($dir, $regex, $subdir, $finddir, $files, $total) = @_;
    my @dir = ();
    map {
        my $isfile = -f();
        my $isdir  = !$isfile && -d();
        if ($finddir ? $isdir : $isfile) {
            ++$$total;
            push(@$files, $_) if File::Basename::basename($_) =~ /$regex/;
        }
        push(@dir, $_) if $isdir && $subdir;
    } sort({lc($a) cmp lc($b)} <$dir/*>);
    map { Search($_, $regex, 1, $finddir, @$files, $$total) } @dir;
}

sub Find($$$$\$)
{
    my ($dir, $filepat, $subdir, $finddir, $total) = @_;
    ($dir, $$total) = (GetAbsDirname($dir), 0);
    my ($regex, @files) = ("", ());
    if ($filepat =~ /^\//) {
        $regex = eval("qr$filepat");
        $filepat = "";
    } else {
        $regex = join("|", map('^' . Wcard2Restr($_) . '$', split(/\s+/, $filepat)));
        $regex = qr/$regex/i;
    }
    DPrint(16, "Find" . ($finddir ? "Dir" : "File") . ": Directory `$dir'" . ($subdir ? " and subdirectories" : "") .
        ", pattern `" . ($filepat ne "" ? "$filepat' $regex" : "$regex'"));
    Search($dir, $regex, $subdir, $finddir, @files, $$total);
...
sub FindDir($$$)
{
    push(@gFindresult, Find(shift(), shift(), shift(), 1, local $_));
}
...
sub FindFile($$$)
{
    push(@gFindresult, Find(shift(), shift(), shift(), 0, local $_));
}
...
sub Zip($$$$@)
{
    ...
    my @files = ($dir ? map(GetAbsDirname($_), grep(-d(), @_)) :
        map(GetAbsFname($_), grep(-f(), @_)));
    $opt = (defined($opt) ? ", options: `$opt'" : "");
    $prefix = GetAbsDirname($prefix) if $prefix ne "";
    DPrint(16, ($dir ? "ZipDir: `$zipfile'$opt, " . @files . " directories" :
        "ZipFile: `$zipfile'$opt, " . @files . " files") . ($prefix ? ", prefix: $prefix\n" : "\n"));
    Archive::Zip::setErrorHandler(sub{});
    my ($error, $zip) = (0, Archive::Zip->new());
...
sub RunICmd()
{
    ...
    while (defined($cmd = GetICmd())) {
        ...
        elsif (/^(cd|copy(dir)?|del(dir)?|find(dir)?(-r)?|headb|logfile|mkcd|mkdir|move|tailb|test|touch|type[bu]?|unzip|workdir|write[bu]?|zip(dir)?(-[jq]+)?)$/) {
            my @files = GlobFiles(GetIPar());
            my $par1 = GetIPar() if /^(?:copy|find|head|move|tail|touch|(un)?zip|write)/;
            my $par2 = GetIPar() if /^(?:head|tail|zip)/;
            @gFindresult = () if /find/;
            next if $gPrintcmd;
            foreach $file (@files) {
                ChangeDir($file)                           if /^cd/;
                DeleteDir($file)                           if /deldir/;
                FindDir($file, $par1, $1)                  if /finddir(-r)?/;
                ...
                FindFile($file, $par1, $1)                 if /find(-r)?$/;
                ...
                WriteFile($file, $par1, $1)                if /write(b|u)?/;
