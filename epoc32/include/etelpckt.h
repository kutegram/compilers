...
The total number of bytes sent is: iOverflowCounterSent*(2^32)+iOverflowCounterSent */
	...
	*/
	...
	public:
		...
		TFileName iCsy;		
		/**Context (data channel) identity. iPort was initially deprecated but has been un-deprecated now.
		This is now passing the information which was earlier being passed by iChannelId required by the lower layers.
		It holds both the Csy name and the Port name.*/	
		TName iPort;			
		/**Context Identity.
		@deprecated This is not needed anymore. The lower layers (Nif) asked for the iPort field instead of iChannelId.
		Hence, all the information is now passed in iPort.*/
		TName iChannelId;	
		};
	/** 
